class Zen {
    removeShortsFromNavbar() {
        ...
        if (iPadShorts) {
            iPadShorts.parentElement.remove()
        }
        const desktopShorts = document.querySelector('ytd-guide-section-renderer a[title="Shorts"]')
        if (desktopShorts) {
            desktopShorts.parentElement.remove()
        }
    }
    
    getYtGrid() {
        return document.querySelector(this.isMobile() ? 'ytm-rich-grid-renderer' : 'ytd-rich-grid-renderer')
    }
    
    isMobile() {
        return location.href.match(/m\.youtube/)
    }
    
    isHomepage() {
        return location.href.match(/youtube\.[^\/.]+\/?$/)
    }

    createContainer() {
        if (document.querySelector('.zen-mode-container')) return
        const interval = setInterval(() => {
            const ytGrid = this.getYtGrid()
            console.info('Zen Mode waiting for YouTube to load...')
            if (!ytGrid) return
            clearInterval(interval)
            this.createZenModeContainer()
            this.makeContainerVisible()
    ...
    makeContainerVisible() {
        ...
        if (!document.querySelector('.zen-mode-container') && isHomepage) this.createZenModeContainer()
        document.querySelector('.zen-mode-container').style.visibility = isHomepage ? 'visible' : 'hidden'
    }
    
    createZenModeContainer() {
        const ytGrid = this.getYtGrid()
        if (document.querySelector('.zen-mode-container')) return
        this.container = document.createElement('div')
        this.container.innerHTML = `
        <h1 class="zen-mode-container" style="min-height: 50vh">
            ...
            <br>
            <span class="zen-mode-title">
                <div>Keep your mind</div>
                <div>peaceful</div>
            </span>
        </h1>
        `
        console.log('child appended zen mode container')
    ...
    setZenMode() {
        ...
        setInterval(() => {
            if (this.isHomepage()) this.createZenModeContainer()
            this.removeShortsFromNavbar()
            if (location.href.match(/youtube\..*shorts/)) history.back()
        }, 500)
        this.setCustomStyle()
        this.setHomepageStyle()
    }
    ...
    setHomepageStyle() {
        if (this.isHomepage()) {
            ...
            this.zenStyleHomepage.innerHTML = `
                ...
                ytm-single-column-browse-results-renderer .rich-grid-renderer-header {
                    display: none !important;
                }
                ytm-single-column-browse-results-renderer .rich-grid-renderer-contents {
                    display: none !important;
                }
            `
            console.log('child appended home')
    ...
    setCustomStyle() {
        ...
        chrome.storage.local.get(['css'], (res) => {
            ...
            console.log('child appended custom style')
